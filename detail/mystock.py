...
def get_ndays_average_shanghai_pb_dif(df,ndays):
...
def monimingxi(stockcode,dif_pb_dict,stock_df,justvalue=0,buycycle=5):
    ...
    for i in range(len(stock_df)):
        ...
        if i%buycycle == 0:
            ...
            if i==0:
                begindate = trade_date
            if trade_date in dif_pb_dict:
                dif_pb = round((float(dif_pb_dict[trade_date]) + justvalue),2)
            else:
                continue
...
def monimingxi1(stockcode,startdate,ndays,justvalue=0,buycycle=5):
    ...
    df = monimingxi(stockcode,history_pb_dif_dict,stock_df,justvalue,1)
    return df
