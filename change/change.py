...
maxdif_df = pd.read_csv('D:/mypython/my/change/maxdif.csv',dtype={'stockcode':object,'maxdif':float})
maxdif_list = maxdif_df.values.tolist()

message = ''

for row in maxdif_list:
    stockcode = row[0]
    maxdif = row[1]
    ...
    if df.at[len(df)-1,'mindif'] <= maxdif:
        message += '\n%s出现机会了'%stockcode
